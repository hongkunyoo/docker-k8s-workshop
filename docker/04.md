...
$ mysql -h $(hostname -i) -u ubuntu -p 1234
```

#### 정답

```bash
docker run -e MYSQL_ROOT_PASSWORD=1234 -e MYSQL_USER=ubuntu -e MYSQL_PASSWORD=1234 -p 3306:3306 mysql
```
...
```bash
docker run -e MYSQL_HOST=$(hostname -i) -e MYSQL_USER=ubuntu -e MYSQL_PASSWORD=1234 -p 5000:5000 app
```
...
```Makefile
start-db:
        docker run -e MYSQL_ROOT_PASSWORD=1234 -e MYSQL_USER=ubuntu -e MYSQL_PASSWORD=1234 -p 3306:3306 --name mysql -d mysql
...
start-app:
        docker run -e MYSQL_HOST=172.17.0.1 -e MYSQL_USER=ubuntu -e MYSQL_PASSWORD=1234 -p 5000:5000 --name app -d app

stop-app:
        docker stop app

start-all: start-db start-app

stop-all: stop-db stop-app
```