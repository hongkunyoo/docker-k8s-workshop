...
```bash
# mynginx-env.yaml 파일을 이용하여 Pod 생성
kubectl apply -f mynginx-env.yaml
...
```bash
kubectl apply -f mynginx-volume.yaml

kubectl exec mynginx-volume -- ls -al /test-volume
# total 12
...
어떻게 하면 `READY`가 `1/1`로 바뀔까요? `mynginx-ready-cmd` 컨테이너에 어떤 명령을 수행하면 됩니다.
...
#### 7. health check & init container & volume

1. init container와 main container의 `/tmp` 위치에 볼륨을 서로 연결해주세요.
2. init container에서 git `https://github.com/kubernetes/kubernetes.git` 레포지토리를 `/tmp/kubernetes` 위치에 클론해주세요.
3. main container의 `readinessProbe`가 `/tmp/kubernetes` 위치에 디렉토리가 생성되었는지 확인하도록 해주세요.
